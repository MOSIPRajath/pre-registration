...
public class AvailabilityUtil {
	...
	public List<String> getHolidayListMasterData(RegistrationCenterDto regDto, HttpHeaders headers) {
		...
		try {
			...
			log.info("sessionId", "idType", "id",
					"In callGetHolidayListRestService method with URL of exceptional holidays-  " + uriBuilder2);
			...
			if (responseEntity2.getBody().getErrors() != null && !responseEntity2.getBody().getErrors().isEmpty()) {
				log.info("sessionId", "idType", "id", "No exceptional holidays are available ");
			} else if (!responseEntity2.getBody().getResponse().getExceptionalHolidayList().isEmpty()) {
